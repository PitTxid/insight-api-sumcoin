...
StatusController.prototype.getTotalSupply = function(callback) {
  this.node.services.bitcoind.getInfo(function(err, info) {
    ...
    function calculateSupplySumcoin (blockHeight) {
      var epochs = Math.floor((blockHeight - 1) / 2102400)
      var remainder = (blockHeight - 1) % 2102400
      var previousEpochsTotalReward = 0
      for (var epoch = 0; epoch < epochs; epoch++) {
    ...
    function calculateSupplyNew (blockHeight) {
      if (blockHeight < 2167200) {
        ...
        return calculateSupply(2102400) + 10000000 + 30 + calculateSupplySumcoin(blockHeight - 2167200);
      }
      
    }
    callback(null, {
      supply: calculateSupplyNew(info.blocks)
    });
  });
};
