...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  transformed.valueOut = transaction.outputSatoshis / 1e6;
  transformed.size = transaction.hex.length / 2; // in bytes
  if (!transaction.coinbase) {
    transformed.valueIn = transaction.inputSatoshis / 1e6;
    transformed.fees = transaction.feeSatoshis / 1e6;
  }
...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  transformed.value = input.satoshis / 1e6;
...
TxController.prototype.transformOutput = function(options, output, index) {
  var transformed = {
    value: (output.satoshis / 1e6).toFixed(6),
    n: index,
    scriptPubKey: {
      hex: output.script
    }
...
TxController.prototype.transformJoinSplit = function(options, jsdesc, index) {
  var transformed = {
    vpub_old: (jsdesc.oldZatoshis / 1e6).toFixed(6),
    vpub_new: (jsdesc.newZatoshis / 1e6).toFixed(6),
    n: index,
  };
  return transformed;
};
